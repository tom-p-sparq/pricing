...
/**
 * Implements a logistic (or logit) conversion model parameterized by price elasticity.
...
*/
export class LogisticConversionModel extends BaseConversionModel {
  /**
   ...
   * @returns {LogisticConversionModel} A new instance of the conversion model.
   */
  static fromReference({ price, conversion, elasticity }) {
    LogisticConversionModel._checkReference(price, conversion, elasticity)
    const b = elasticity / ((1 - conversion) * price)
    const a = Math.log(conversion) - Math.log(1 - conversion) - b * price
    return new LogisticConversionModel({ a, b })
  }

  /**
   * Creates a new model instance by interpolating between two points.
   * This method calculates the shape of the logistic conversion curve that passes
   * through two given points (p0, c0) and (p1, c1), and then creates a new
   * `LogisticConversionModel` instance.
   * @override
   ...
   * @returns {LogisticConversionModel}
   */
  static fromFlat(averageConversion) {
    return new LogisticConversionModel({ a: Math.log(averageConversion / (1 - averageConversion)), b: 0 });
  }
